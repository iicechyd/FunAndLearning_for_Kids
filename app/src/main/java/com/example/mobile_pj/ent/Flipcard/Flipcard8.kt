...
class Flipcard8 : AppCompatActivity() {
    var back : Button? = null
    private lateinit var cardImg: ImageView
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_flipcard8)
        init()
        ...
        oa1.addListener(object : AnimatorListenerAdapter(){
            override fun onAnimationEnd(animation: Animator) {
                ...
                if (cardImg.drawable.constantState == resources.
                    getDrawable(R.drawable.flip_b8).constantState){
                    cardImg.setImageResource(R.drawable.flip_f8)
                }else{
                    cardImg.setImageResource(R.drawable.flip_b8)
                }
                oa2.start()
            }
        })
